            ...
            for _, best_row in _met_keys.iterrows():
                ...
                for _sym in _all_syms[:5]:
                    ...
                    _exit_rows.append({
                        ...
                        "Entry Date":     _first_spotted.strftime("%d %b %Y") if _first_spotted is not None and not pd.isna(_first_spotted) else "—",
                        "Entry Price":    f"${_entry_price:.2f}" if _entry_price else "—",
                        "Media Pickup":   _pickup_date.strftime("%d %b %Y") if _pickup_date is not None and not pd.isna(_pickup_date) else "—",
                        "Exit Date":      _exit_date.strftime("%d %b %Y") if _exit_date is not None and not pd.isna(_exit_date) else "—",
